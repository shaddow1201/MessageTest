...
/** @title CredentialOrgFactoryB  */
import "./Pausable.sol";
import "./SafeMath32.sol";

contract CredentialOrgFactoryB is Pausable {
    ...
    */
    ...
    returns (bool createStatus)
    {
        ...
        require(bytes(_shortName).length > 0 && bytes(_shortName).length < 31);
        require(bytes(_officialSchoolName).length > 0 && bytes(_officialSchoolName).length < 70);
        require(_schoolAddress != 0);        
        createStatus = false;
    ...
    */
    ...
    returns (string shortName, string officialSchoolName, address schoolAddress)
    {
        ...
        officialSchoolName = "";
        schoolAddress = 0;
        require(_credentialOrgPosition >= 0);
